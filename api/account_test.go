...
import (
	...
	mockdb "github.com/cryocooler/simplebank/db/mock"
...
// 			buildStubs: func(store *mockdb.MockStore) {
// 				store.EXPECT().UpdateAccount(gomock.Any(), gomock.Eq(account.ID)).
// 					Times(1).
// 					Return(db.Account{ID: account.ID,
// 						Owner:     account.Owner,
// 						Balance:   account.Balance + 10,
...
// 		payload := fmt.Sprintf(`{"balance": %d}`, tc.balance)
